...
export default function EmailGenerator({
  ...
  function handleGenerate() {
    const username = customPrefix.trim() ? customPrefix.trim() : undefined;
  ...
  function generateRandomUsername() {
    ...
    // 自动触发生成
    onGenerate(result, selectedDomain || undefined);
  }
    ...
    <>
      ...
      <header className="flex flex-col items-center pt-16 pb-10 px-4 text-center">
        ...
        <div className="mt-6 flex flex-wrap items-center justify-center gap-3">
          <button
            ...
            disabled={loading}
            className="relative inline-flex items-center justify-center rounded-full bg-gradient-to-r from-cyan-500 to-blue-600 px-6 py-2.5 text-base font-semibold text-white shadow-lg btn-glow transition hover:scale-105 disabled:opacity-60 disabled:cursor-not-allowed"
          >
            {loading ? (
          ...
          {email && (
            <button
              onClick={onCopy}
              className="rounded-lg bg-slate-700/80 hover:bg-slate-600 px-5 py-2.5 text-base font-medium btn-glow transition"
            >
          ...
          <button
            ...
            disabled={loading}
            className="rounded-lg bg-slate-700/80 hover:bg-slate-600 px-5 py-2.5 text-base font-medium btn-glow transition disabled:opacity-60 disabled:cursor-not-allowed"
          >
            Random
          </button>
        </div>
      ...
      {email && (
        <section className="flex flex-col items-center px-4 pb-8">
          <div
            className={`flex items-center justify-center rounded-2xl px-8 py-6 max-w-full transition-all duration-500 ${
              emailHighlight
          ...
          >
            <span className="text-2xl sm:text-3xl font-mono text-cyan-300 truncate">
              {email.address}
            </span>
          </div>
